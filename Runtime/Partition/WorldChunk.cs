...
namespace ShoelaceStudios.GridSystem
{
    public class WorldChunk : AbstractChunk
    {
        ...
        {
            cachedMask ??= BuildChunkMask();
            FloodFillParams masked = FloodFillParams.WithRegionMask(
                cachedMask, parameters.StopAtWalls, parameters.Diagonals,
                parameters.TrackDepth, parameters.EdgeSystem);
