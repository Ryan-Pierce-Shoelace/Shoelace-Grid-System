...
namespace ShoelaceStudios.GridSystem.Utils
{
	public readonly struct GridBounds
	{
		...
		public static GridBounds FromWorldBounds(Bounds worldBounds, float cellSize, Vector3 gridOrigin)
		{
			return new GridBounds(
				Mathf.FloorToInt((worldBounds.min.x - gridOrigin.x) / cellSize),
				Mathf.CeilToInt((worldBounds.max.x - gridOrigin.x) / cellSize),
				Mathf.FloorToInt((worldBounds.min.y - gridOrigin.y) / cellSize),
				Mathf.CeilToInt((worldBounds.max.y - gridOrigin.y) / cellSize));
		}
	}
}